...
for file in os.listdir(videos_dir):
    ...
    if file.endswith('.mp4'):
        ...
        if refined_shot_bnd_frms_vid and (f_end_shots[-1] <= (indmax - min_shot_len)):  # matrix[-1] = the last element of the matrix
            ...
            #print("f_start_shots",f_start_shots)
            #print("f_end_shots",f_end_shots)

        """挑选所需节点"""
        img_path_aim=r"MoblieVit/data/to_classifd"
        print(f_end_shots)
        for i in range(len(f_start_shots)) :
            #img_path_fortest = r"videos_test/frames_BAR_RMA_GOAL/frame{}.jpg".format(i)
            img_path=r"videos/frames_"+video_name.split('.mp4')[0]+"/frame{}.jpg".format(f_start_shots[i])
            shutil.copy(img_path,img_path_aim)
            mid=f_start_shots[i]+(f_end_shots[i]-f_start_shots[i])//2
            img_path=r"videos/frames_"+video_name.split('.mp4')[0]+"/frame{}.jpg".format(mid)
            shutil.copy(img_path,img_path_aim)

        """predict"""
        os.system('python MoblieVit/predict.py')

        """redefine the fss fes"""
        f_start_shots_tmp=[]
        f_end_shots_tmp=[]
        f_start_shots_copy=[]
        shot_num_start=[]
        #文件夹路径->img_path_aim
        img_path_clfd=r"D:\zstudy\Projec_badminton_video_segmetation\Shot-Transition-Detection-main\MoblieVit\data\classifd"
        shot_num_start=listdir(img_path_clfd,shot_num_start)
        print("shot_num_start:",shot_num_start)
        ####################################
        for i in shot_num_start:
            jpg_name = i.split("\\")[-1]
            jpg_num = re.findall(r"\d+",jpg_name)
            #print(jpg_num)
            f_start_shots_copy.append(int(jpg_num[0]))
        count=0
        #print("f_start_shots",f_start_shots)
        #print("f_start_shots_copy",f_start_shots_copy)
        for i in f_start_shots:
            if  i in f_start_shots_copy:
                f_start_shots_tmp.append(f_start_shots[count])
                f_end_shots_tmp.append(f_end_shots[count])
            count+=1
        f_start_shots=f_start_shots_tmp
        f_end_shots=f_end_shots_tmp
        print("f_start_shots",f_start_shots)
        print("f_end_shots",f_end_shots)
        ...
        for i in range(0, len(f_start_shots)):
            ...
            shots_dir = videos_dir + r'/videoShots_' + video_name.split('.mp4')[0] + '/'
            if not os.path.exists(shots_dir):
                os.makedirs(shots_dir)
            #print("frames_dir:",frames_dir)
            #print("shots_dir:",shots_dir)
        ...
        csvfile_name = r'/videoShotsInfo_'+ video_name.split('.mp4')[0] + '.csv'
        data_out = {}
        ...
        shots_dir = r"videos_test"
        df_out.to_csv(shots_dir + "\info.csv", index=True)
